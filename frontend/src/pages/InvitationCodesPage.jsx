...
const InvitationCodesPage = () => {
  ...
  const handleGenerate = async (e) => {
    e.preventDefault();
    setGenerating(true);
    setNewCodes([]); // Очищаем предыдущие коды
    ...
    try {
      ...
      if (data && data.codes && data.codes.length > 0) {
        ...
        await loadCodes();
        alert(`Сгенерировано ${data.count || data.codes.length} кодов`);
      } else {
        alert('Не удалось сгенерировать коды. Проверьте подключение к серверу.');
      }
    } catch (error) {
      console.error('Generate codes error:', error);
      alert('Ошибка при генерации кодов: ' + (error.message || 'Неизвестная ошибка'));
    } finally {
      setGenerating(false);
    }
  };
