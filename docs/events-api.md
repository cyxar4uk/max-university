...
В MAX мероприятия отображаются в виджетах на Главной и в Хабе. Данные берутся из внешнего API проекта ивентов; виджеты показывают краткую информацию и ссылку «Открыть в боте» — бот [События РАНХиГС](https://t.me/event_ranepa_bot).

## Какое API нужно реализовать в проекте мероприятий

В вашем проекте по мероприятиям (ивентам) нужно реализовать HTTP API, которое MAX будет вызывать через свой бэкенд (прокси). Достаточно одного метода:

- **GET /events** (или эквивалент по вашему базовому URL).

**Параметры запроса (query):**

- `limit` (optional) — количество записей (по умолчанию MAX передаёт 10).
...
Рекомендуемые поля для карточки в виджете: `id`, `title` или `name`, `date` (ISO 8601), `location`, `registration_status`. Ссылка на бота в MAX по умолчанию: **https://t.me/event_ranepa_bot** (переопределяется через `EVENTS_BOT_LINK` на бэкенде MAX).
...
- **EVENTS_BOT_LINK** — ссылка на бота (по умолчанию `https://t.me/event_ranepa_bot` — События РАНХиГС).
...
4. **Ссылка на бота**
   - Один общий URL бота для всех виджетов или отдельная ссылка на конкретное мероприятие (deep link)? Если deep link — какой формат (например, `https://t.me/event_ranepa_bot?start=event_123`)? В MAX по умолчанию используется бот [События РАНХиГС](https://t.me/event_ranepa_bot).
